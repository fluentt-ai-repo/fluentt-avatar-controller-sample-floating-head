        ...
        public override void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            if (controller == null)
            {
                ...
                if (controller == null)
                {
                    if (!hasShownError)
                    {
                        Debug.LogError("[TalkingAnimationRandomizer] Controller not found! Make sure FluentTAvatarControllerFloatingHead is attached to the same GameObject as the Animator. This behaviour will be disabled.");
                        hasShownError = true;
